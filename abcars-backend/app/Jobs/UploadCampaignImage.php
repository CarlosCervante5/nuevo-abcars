...
class UploadCampaignImage
{
    ...
    public function __construct( String $path, MarketingCampaign $campaign, String $original_filename)
    {
        ...
        $this->base_folder = env('AWS_CAMPAIGN_FOLDER_BASE', 'default_folder');
    ...
    {   
        ...
        try {
            ...
            $cloudinary_file = $cloudinary->uploadApi()->upload(storage_path('app/' . $this->path), [
                'public_id' => $name,
                'folder' => $this->base_folder . '/' . $this->campaign->uuid,
                'transformation' => [
                    'quality' => 'auto',
                    'fetch_format' => 'jpg'
                ]
            ...
            $s3_path = $this->base_folder . '/' . $this->campaign->uuid . '/' . $name . '.jpg';
            ...
            if ($s3_result) {
                ...
                $this->campaign->update(['image_path' => $this->aws_url . '/' . $s3_path]);
