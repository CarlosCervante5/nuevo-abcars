...
class UploadRewardImage
{
    ...
    public function __construct( String $path, Reward $reward, String $original_filename)
    {
        ...
        $this->base_folder = env('AWS_REWARD_FOLDER_BASE', 'default_folder');
    ...
    {   
        ...
        try {
            ...
            $cloudinary_file = $cloudinary->uploadApi()->upload(storage_path('app/' . $this->path), [
                'public_id' => $name,
                'folder' => $this->base_folder . '/' . $this->reward->uuid,
                'transformation' => [
                    'quality' => 'auto',
                    'fetch_format' => 'jpg'
                ]
            ...
            $s3_path = $this->base_folder . '/' . $this->reward->uuid . '/' . $name . '.jpg';
            ...
            if ($s3_result) {
                ...
                $this->reward->update(['image_path' => $this->aws_url . '/' . $s3_path]);
